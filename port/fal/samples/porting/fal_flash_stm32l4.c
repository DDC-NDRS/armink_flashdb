...
static int write(long offset, uint8_t const* buf, size_t size) {
    size_t   i;
    uint32_t addr = stm32_onchip_flash.addr + offset;
    ...
    */
    ...
    for (i = 0; i < size; i += 8, buf += 8, addr += 8) {
        memcpy(
            &write_data, buf,
            8); // The third parameter used to ensure HAL_FLASH_Program is the alignment of the first address in memory
...
static int erase(long offset, size_t size) {
    ...
    for (i = 0; i < erase_pages; i++) {
        ...
        if (flash_status != HAL_OK) {
            ...
            // FLash operation can be very time-consuming, if there is a watchdog that needs to feed the dog, the
            // following code is implemented by the user
            feed_dog();
        }
    }
    HAL_FLASH_Lock();
