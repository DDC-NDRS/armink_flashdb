...
static void update_sector_cache(fdb_kvdb_t db, kv_sec_info_t sector) {
    size_t empty_index = FDB_SECTOR_CACHE_TABLE_SIZE;

    for (size_t i = 0; i < FDB_SECTOR_CACHE_TABLE_SIZE; i++) {
...
static void update_kv_cache(fdb_kvdb_t db, char const* name, size_t name_len, uint32_t addr) {
    size_t empty_index = FDB_KV_CACHE_TABLE_SIZE;
    size_t min_activity_index = FDB_KV_CACHE_TABLE_SIZE;
 ...
 */
static bool get_kv_from_cache(fdb_kvdb_t db, char const* name, size_t name_len, uint32_t* addr) {
    ...
    for (size_t i = 0; i < FDB_KV_CACHE_TABLE_SIZE; i++) {
        if ((db->kv_cache_table[i].addr != FDB_DATA_UNUSED) && (db->kv_cache_table[i].name_crc == name_crc)) {
            ...
            _fdb_flash_read((fdb_db_t)db, db->kv_cache_table[i].addr + KV_HDR_DATA_SIZE, (uint32_t*)saved_name,
                            FDB_KV_NAME_MAX);
...
static uint32_t find_next_kv_addr(fdb_kvdb_t db, uint32_t start, uint32_t end) {
    ...
    for (; start < end && start + sizeof(buf) < end; start += (sizeof(buf) - sizeof(uint32_t))) {
        ...
        for (uint32_t i = 0; i < sizeof(buf) - sizeof(uint32_t) && start + i < end; i++) {
...
static fdb_err_t read_sector_info(fdb_kvdb_t db, uint32_t addr, kv_sec_info_t sector, bool traversal) {
    ...
    sector->status.store =
        (fdb_sector_store_status_t)_fdb_get_status(sec_hdr.status_table.store, FDB_SECTOR_STORE_STATUS_NUM);
    sector->status.dirty =
        (fdb_sector_dirty_status_t)_fdb_get_status(sec_hdr.status_table.dirty, FDB_SECTOR_DIRTY_STATUS_NUM);
...
static bool find_kv_no_cache(fdb_kvdb_t db, char const* key, fdb_kv_t kv) {
    bool find_ok = false;
...
static bool find_kv(fdb_kvdb_t db, char const* key, fdb_kv_t kv) {
    bool find_ok = false;
...
static size_t get_kv(fdb_kvdb_t db, char const* key, void* value_buf, size_t buf_len, size_t* value_len) {
    struct fdb_kv kv;
    size_t read_len = 0;
 ...
 */
fdb_kv_t fdb_kv_get_obj(fdb_kvdb_t db, char const* key, fdb_kv_t kv) {
    bool find_ok = false;
 ...
 */
size_t fdb_kv_get_blob(fdb_kvdb_t db, char const* key, fdb_blob_t blob) {
    size_t read_len = 0;
 ...
 * @return value
 */
char* fdb_kv_get(fdb_kvdb_t db, char const* key) {
...
static fdb_err_t format_sector(fdb_kvdb_t db, uint32_t addr, uint32_t combined_value) {
    ...
    if (result == FDB_NO_ERR) {
        ...
        #else // seperate the whole "sec_hdr" program to serval sinle program operation to prevent re-program issue on STM32L4xx
              // or other MCU internal flash
...
static fdb_err_t del_kv(fdb_kvdb_t db, char const* key, fdb_kv_t old_kv, bool complete_del) {
    fdb_err_t result = FDB_NO_ERR;
    uint32_t dirty_status_addr;
    struct fdb_kv kv = {
        .status = FDB_KV_UNUSED
    };
    ...
    if (!complete_del) {
        ...
        result =
            _fdb_write_status((fdb_db_t)db, old_kv->addr.start, status_table, FDB_KV_STATUS_NUM, FDB_KV_DELETED, true);

        if (!db->last_is_complete_del && result == FDB_NO_ERR) {
            #ifdef FDB_KV_USING_CACHE
            /* delete the KV in flash and cache */
            if (key != NULL) {
                /* when using del_kv(db, key, NULL, true) or del_kv(db, key, kv, true) in fdb_del_kv(db, ) and
                 * set_kv(db, ) */
    ...
    if (result == FDB_NO_ERR && _fdb_read_status((fdb_db_t)db, dirty_status_addr, status_table,
                                                 FDB_SECTOR_DIRTY_STATUS_NUM) == FDB_SECTOR_DIRTY_FALSE) {
        result = _fdb_write_status((fdb_db_t)db, dirty_status_addr, status_table, FDB_SECTOR_DIRTY_STATUS_NUM,
...
static bool do_gc(kv_sec_info_t sector, void* arg1, void* arg2) {
    ...
    if (sector->check_ok &&
        (sector->status.dirty == FDB_SECTOR_DIRTY_TRUE || sector->status.dirty == FDB_SECTOR_DIRTY_GC)) {
        uint8_t status_table[FDB_DIRTY_STATUS_TABLE_SIZE];
        /* change the sector status to GC */
        _fdb_write_status((fdb_db_t)db, sector->addr + SECTOR_DIRTY_OFFSET, status_table, FDB_SECTOR_DIRTY_STATUS_NUM,
...
static fdb_err_t align_write(fdb_kvdb_t db, uint32_t addr, uint32_t const* buf, size_t size) {
    fdb_err_t result = FDB_NO_ERR;
    size_t align_remain;
...
static fdb_err_t create_kv_blob(fdb_kvdb_t db, kv_sec_info_t sector, char const* key, void const* value, size_t len) {
    ...
    if (kv_addr != FAILED_ADDR || (kv_addr = new_kv(db, sector, kv_hdr.len)) != FAILED_ADDR) {
        ...
        if (result == FDB_NO_ERR) {
            result = _fdb_write_status((fdb_db_t)db, kv_addr, kv_hdr.status_table,
 ...
 * @return result
 */
fdb_err_t fdb_kv_del(fdb_kvdb_t db, char const* key) {
    fdb_err_t result = FDB_NO_ERR;
...
static fdb_err_t set_kv(fdb_kvdb_t db, char const* key, void const* value_buf, size_t buf_len) {
    fdb_err_t result = FDB_NO_ERR;
 ...
 * @return result
 */
fdb_err_t fdb_kv_set_blob(fdb_kvdb_t db, char const* key, fdb_blob_t blob) {
    fdb_err_t result = FDB_NO_ERR;
 ...
 * @return result
 */
fdb_err_t fdb_kvdb_init(fdb_kvdb_t db, char const* name, char const* path, struct fdb_default_kv* default_kv,
                        void* user_data) {
    fdb_err_t result = FDB_NO_ERR;
    struct kvdb_sec_info sector;
